...
```bash
go mod init gitlab.com/redha/goprotobuf
...
```bash
protoc --go_out=. --go-grpc_out=. proto/invoicer.proto
```
OR this -

```bash
protoc --go_out=. --go_opt=module=gitlab.com/redha/goprotobuf --go-grpc_out=. --go-grpc_opt=module=gitlab.com/redha/goprotobuf proto/invoicer.proto
```
